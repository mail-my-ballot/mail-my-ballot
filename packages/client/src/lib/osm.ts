...
export const osmGeocode = async (addr: string): Promise<Locale> => {
  const result = await fetch(`https://nominatim.openstreetmap.org/search/${addr}?format=json&countrycodes=us`)
